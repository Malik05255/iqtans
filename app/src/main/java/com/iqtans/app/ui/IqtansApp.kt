...
fun IqtansApp(repository: DealRepository, liveClient: BackendSearchClient? = null) {
    ...
    var cards by remember { mutableStateOf<List<UserPaymentCard>>(emptyList()) }
            ...
            Box(Modifier.fillMaxSize().padding(innerPadding)) {
                when (screen) {
                    ...
                    AppScreen.HOME -> HomeScreen(
                        ...
                        onSearch = { newRequest -> request = newRequest; screenName = AppScreen.RESULTS.name },
                    ...
                    AppScreen.HOTEL -> {
                        ...
                        else HotelDetailScreen(
                            hotel = hotel,
                            watched = hotel.id in watchedIds,
                            onBack = { screenName = AppScreen.RESULTS.name },
                            onToggleWatch = {
                                hotelSnapshots[hotel.id] = hotel
                                if (hotel.id in watchedIds) watchedIds.remove(hotel.id) else watchedIds.add(hotel.id)
                            },
                    ...
                    AppScreen.DEAL -> {
                        ...
                        else DealDetailScreen(hotelName = hotel.name, offer = offer, onBack = { screenName = AppScreen.HOTEL.name })
                    }
                    AppScreen.CARDS -> CardsScreen(cards = cards, onCardsChange = { cards = it })
                    AppScreen.WATCHLIST -> WatchlistScreen(
                        hotels = watchedIds.mapNotNull { hotelSnapshots[it] ?: repository.getHotel(it) },
                        onHotelClick = ::selectHotel
                    )
                }
        ...
        BottomDestination.entries.forEach { item ->
            NavigationBarItem(
                ...
                colors = NavigationBarItemDefaults.colors(
                    selectedIconColor = Emerald, selectedTextColor = Emerald,
                    indicatorColor = Emerald.copy(alpha = .10f), unselectedIconColor = Muted, unselectedTextColor = Muted
                )
            )
        }
    }
