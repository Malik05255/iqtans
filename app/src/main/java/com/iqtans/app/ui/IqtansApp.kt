...
@Composable
fun IqtansApp(repository: DealRepository, liveClient: BackendSearchClient? = null) {
    ...
    val watchedIds = remember { mutableStateListOf<String>() }; val hotelSnapshots = remember { mutableStateMapOf<String, HotelDeal>() }
    var cards by remember { mutableStateOf<List<UserPaymentCard>>(emptyList()) }; var liveEnvelope by remember { mutableStateOf<LiveSearchEnvelope?>(null) }
    var loading by remember { mutableStateOf(false) }; var loadError by remember { mutableStateOf<String?>(null) }
    ...
    CompositionLocalProvider(LocalLayoutDirection provides LayoutDirection.Rtl) {
        Scaffold(
            ...
            bottomBar = { if (screen in setOf(AppScreen.HOME, AppScreen.RESULTS, AppScreen.CARDS, AppScreen.WATCHLIST)) IqtansBottomBar(selected = screen, onSelect = { screenName = it.name }) }
        ) { innerPadding ->
            Box(Modifier.fillMaxSize().padding(innerPadding)) {
                when (screen) {
                    ...
                    AppScreen.HOTEL -> {
                        ...
                        else HotelDetailScreen(hotel, hotel.id in watchedIds, { screenName = AppScreen.RESULTS.name }, {
                            hotelSnapshots[hotel.id] = hotel; if (hotel.id in watchedIds) watchedIds.remove(hotel.id) else watchedIds.add(hotel.id)
                    ...
                    AppScreen.WATCHLIST -> WatchlistScreen(hotels = watchedIds.mapNotNull { hotelSnapshots[it] ?: repository.getHotel(it) }, onHotelClick = ::selectHotel)
                }
                ...
                if (status != null && screen != AppScreen.CITY) Surface(modifier = Modifier.align(Alignment.TopCenter).padding(top = 8.dp, start = 18.dp, end = 18.dp), shape = MaterialTheme.shapes.medium, color = MaterialTheme.colorScheme.errorContainer, shadowElevation = 2.dp) {
...
@Composable private fun IqtansBottomBar(selected: AppScreen, onSelect: (AppScreen) -> Unit) {
    NavigationBar(containerColor = MaterialTheme.colorScheme.surface, tonalElevation = 0.dp) {
        BottomDestination.entries.forEach { item -> NavigationBarItem(selected = selected == item.screen, onClick = { onSelect(item.screen) }, icon = { Icon(item.icon, item.label) }, label = { Text(item.label, fontSize = 11.sp) }, colors = NavigationBarItemDefaults.colors(selectedIconColor = Emerald, selectedTextColor = Emerald, indicatorColor = Emerald.copy(alpha = .10f), unselectedIconColor = Muted, unselectedTextColor = Muted)) }
    }
}
