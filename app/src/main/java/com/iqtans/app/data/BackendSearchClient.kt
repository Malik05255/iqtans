...
class BackendSearchClient(private val baseUrl: String) {
    suspend fun search(request: SearchRequest, cards: List<UserPaymentCard>): LiveSearchEnvelope = withContext(Dispatchers.IO) {
        val endpoint = URL(baseUrl.trimEnd('/') + "/v1/search")
    ...
    private fun parseHotel(hotel: JSONObject): HotelDeal? {
        ...
        val offers = mutableListOf<DealOffer>()
        var firstRaw: JSONObject? = null
        rawOffers.forEachObject { raw -> if (firstRaw == null) firstRaw = raw; offers += parseOffer(raw, baseline) }
        if (offers.isEmpty()) return null
        val first = firstRaw ?: JSONObject()
        val discoveries = mutableListOf<DiscoveryHint>()
        ...
        val flexibleDateLabel = flexible?.let {
            val inDate = it.optString("checkIn")
            val outDate = it.optString("checkOut")
            if (inDate.isNotBlank() && outDate.isNotBlank()) "$inDate — $outDate" else null
        }
        val rating = first.optDouble("rating", 0.0)
        val ratingCount = first.optInt("ratingCount", 0)
        val provider = first.optString("provider", "مصدر حي")
        return HotelDeal(
            id = hotel.optString("key", hotel.optString("name")), name = hotel.optString("name", "فندق"), city = hotel.optString("city"),
            area = "نتيجة اقتنص الحية", stars = first.optInt("stars", 0).coerceIn(0, 5), rating = rating, ratingCount = ratingCount,
            distanceLabel = provider, reviewSources = if (rating > 0) listOf(ReviewSource(provider, rating, 10, ratingCount)) else emptyList(),
            offers = offers, insight = discoveries.firstOrNull()?.title ?: offers.first().method,
            flexibilitySaving = flexibilitySaving, flexibleDateLabel = flexibleDateLabel, discoveries = discoveries
        )
    }

    private fun parseOffer(raw: JSONObject, baseline: Int): DealOffer {
        val total = raw.optDouble("totalPrice", 0.0).roundToInt()
        val requirements = raw.optJSONArray("requirements").strings()
        val breakdownObject = raw.optJSONObject("breakdown") ?: JSONObject()
        val breakdown = buildList {
            val room = breakdownObject.optDouble("room", 0.0).roundToInt(); val taxes = breakdownObject.optDouble("taxes", 0.0).roundToInt()
            val fees = breakdownObject.optDouble("mandatoryFees", 0.0).roundToInt(); val property = breakdownObject.optDouble("payAtProperty", 0.0).roundToInt()
            if (room > 0) add("سعر الإقامة الأساسي" to room)
            if (taxes > 0) add("ضرائب" to taxes)
            if (fees > 0) add("ضرائب/رسوم إلزامية ضمن الإجمالي" to fees)
            if (property > 0) add("من الإجمالي يُدفع في الفندق" to property)
        }
        ...
        val steps = buildList {
            add("افتح مصدر الحجز: $source."); addAll(requirements)
            add("تأكد أن نفس الفندق والتواريخ والغرفة والسياسات ما زالت مطابقة قبل الدفع.")
            add("تحقق من أن الإجمالي النهائي الظاهر لا يتجاوز $total ${breakdownObject.optString("currency", "SAR")}.")
        }
        return DealOffer(
            id = raw.optString("id"), source = source, finalPrice = total, referencePrice = if (baseline > 0) baseline else total,
            currency = breakdownObject.optString("currency", "SAR"), title = title, method = raw.optString("method", source),
            trust = if (verification == "LIVE_VERIFIED") PriceTrust.VERIFIED_LIVE else PriceTrust.DISCOVERED,
            matchPercent = raw.optInt("matchPercent", 100), lastChecked = raw.optString("verifiedAt", "تحقق حديث"),
            cancellation = raw.optString("cancellation", "راجع سياسة الإلغاء"), meal = raw.optString("meal", "حسب العرض"),
            conditions = requirements, steps = steps, breakdown = breakdown,
            cardRequirement = requirements.firstOrNull { it.contains("بطاقة") }, memberRequirement = requirements.firstOrNull { it.contains("عضوية") || it.contains("حساب") },
